...
class ReportsModerationScreen extends StatefulWidget {
  ...
  @override
  State<ReportsModerationScreen> createState() => _ReportsModerationScreenState();
}
...
class _ReportsModerationScreenState extends State<ReportsModerationScreen> {
      ...
      child: FutureBuilder<List<ScamReport>>(
        ...
        builder: (context, snapshot) {
          ...
          final filteredReports = allReports.where((r) {
            final matchesStatus = _statusFilter == 'All Statuses' || r.verificationStatus == _statusFilter;
            final matchesCategory = _categoryFilter == 'All Categories' || r.category == _categoryFilter;
            ...
            return matchesStatus && matchesCategory && matchesSearch && matchesRisk;
          }).toList();
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    flex: 1,
                    child: _buildMiniMap(filteredReports),
                  ),
                ],
              ),
            ],
  ...
  Widget _buildStatCards(List<ScamReport> reports) {
    final pending = reports.where((r) => r.verificationStatus == 'Pending').length;
    final total = reports.length;
    
    return Row(
      children: [
        _buildStatCard('TOTAL SCAM REPORTS', '$total', 'Across Malaysia', Icons.description_outlined, AppColors.blue),
        const SizedBox(width: 16),
        _buildStatCard('PENDING REVIEW', '$pending', 'Requires attention', Icons.warning_amber_rounded, Colors.orange),
        const SizedBox(width: 16),
        _buildStatCard('VERIFIED FALSE / FAKE', '${reports.where((r) => r.verificationStatus == 'Fake').length}', 'Accuracy Filter', Icons.delete_outline, Colors.red),
        const SizedBox(width: 16),
        _buildStatCard('COMMUNITY REACH', '23.3K', 'Users protected', Icons.people_outline, Colors.green),
      ],
    );
  }
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(title, style: const TextStyle(color: AppColors.slate, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
  ...
  Widget _buildFilters(List<ScamReport> reports) {
    final categories = ['All Categories', ...reports.map((r) => r.category).toSet()];
    return Row(
      children: [
        _buildFilterDropdown('Status: $_statusFilter', ['All Statuses', 'Pending', 'Verified', 'Resolved', 'Fake'], (v) => setState(() => _statusFilter = v!)),
        const SizedBox(width: 12),
        _buildFilterDropdown('Category: $_categoryFilter', categories, (v) => setState(() => _categoryFilter = v!)),
        const Spacer(),
        const Text('Suspicious Only', style: TextStyle(color: AppColors.slate, fontSize: 12, fontWeight: FontWeight.w600)),
        const SizedBox(width: 8),
        Switch(value: _highRiskOnly, onChanged: (v) => setState(() => _highRiskOnly = v), activeColor: AppColors.blue),
      ],
    );
  }

  Widget _buildFilterDropdown(String label, List<String> items, ValueChanged<String?> onChanged) {
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          hint: Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
  ...
  Widget _buildReportsTable(List<ScamReport> reports) {
    if (reports.isEmpty) {
      return const Padding(padding: EdgeInsets.all(48), child: Center(child: Text('No reports found.')));
    }
    return DataTable(
      headingTextStyle: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.slate, fontSize: 11),
      columns: const [
      ...
      rows: reports.map((r) => DataRow(
        cells: [
          DataCell(Text('#${r.id?.substring(0, 4).toUpperCase() ?? "0000"}', style: const TextStyle(fontWeight: FontWeight.bold)), onTap: () => _openDetail(r)),
          ...
          DataCell(Icon(Icons.flag_outlined, color: (r.amountLost ?? 0) > 200 ? Colors.red : AppColors.muted, size: 18)),
        ]
      )).toList(),
    );
  }
  ...
  Widget _buildMiniMap(List<ScamReport> reports) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Area Hotspot Heatmap', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          ...
          AspectRatio(
            ...
            child: ClipRRect(
              ...
              child: FlutterMap(
                options: const MapOptions(initialCenter: LatLng(3.1390, 101.6869), initialZoom: 12),
                children: [
                  TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png'),
                  CircleLayer(circles: reports.map((r) => CircleMarker(
          ...
          const SizedBox(height: 20),
          const Text('LEGEND', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppColors.slate)),
          const SizedBox(height: 12),
          const Wrap(
            spacing: 12,
            runSpacing: 8,
